...
from containers.containers_rstar import Open as Open1
from containers.containers_rstar import Closed as Closed1
...
class Rstar(SearchAlgorithm):
    ...
    def run(self, area: Area, start_coord, goal_coord):
        ...
        def reevaluate(s):
            path[s][bp[s]] = self.astar.run(area, bp[s], s, open_time, closed_time, 4*self.D)
...
class Bounded_WAstar(SearchAlgorithm):
    ...
    def run(self, area: Area, start_coord, goal_coord, open_time, closed_time, max_closed=100):
